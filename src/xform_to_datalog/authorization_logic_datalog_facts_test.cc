...
static const std::vector<absl::string_view> program = {"simple_auth_logic", "empty_auth_logic", "says_owns_tag"};
...
TEST_P(AuthorizationLogicDatalogFactsTest, InvokesRustToolAndGeneratesOutput) {
    ...
    std::cout << "Harsha" << std::endl;
    const std::string path = "src/xform_to_datalog/testdata";
    fs::path test_data_dir = raksha::test_utils::GetTestDataDir(path);
    auto auth_facts =
    ...
    std::vector<std::string> expected_datalog =
        raksha::test_utils::ReadFileLines(test_data_dir / absl::StrCat(param, ".dl"));
